...
export const PartnerConnect: React.FC<PartnerConnectProps> = ({ onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <div className="bg-white dark:bg-emerald-900 rounded-3xl w-full max-w-sm overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
                {/* Header */}
                <div className="flex justify-between items-center p-4 border-b border-gray-100">
                    <h3 className="font-bold text-lg text-gray-900 dark:text-white">Connect Partner</h3>
                ...
                {/* Tabs */}
                <div className="flex p-2 bg-gray-50 dark:bg-emerald-950/50">
                    <button
                        onClick={() => setActiveTab('my-code')}
                        className={`flex-1 py-2 text-sm font-bold rounded-xl transition-all ${activeTab === 'my-code' ? 'bg-white dark:bg-emerald-800 shadow text-emerald-600 dark:text-emerald-100' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400'}`}
                    >
                        My Code
                    </button>
                    <button
                        onClick={() => setActiveTab('scan')}
                        className={`flex-1 py-2 text-sm font-bold rounded-xl transition-all ${activeTab === 'scan' ? 'bg-white dark:bg-emerald-800 shadow text-emerald-600 dark:text-emerald-100' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400'}`}
                    >
                        Scan QR
                    </button>
                </div>
                    ...
                    {activeTab === 'my-code' ? (
                        <div className="flex flex-col items-center text-center space-y-6">
                            ...
                            <div className="text-gray-500 text-sm">
                                ...
                                <div className="flex items-center justify-center gap-2 bg-gray-100 py-2 px-3 rounded-lg max-w-full overflow-hidden">
                                    <span className="truncate font-mono text-xs">{user.uid}</span>
                                    <button onClick={copyToClipboard} className="text-emerald-600">
