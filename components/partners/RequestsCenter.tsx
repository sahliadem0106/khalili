...
export const RequestsCenter: React.FC<RequestsCenterProps> = ({ onRequestProcessed }) => {
    ...
    if (requests.length === 0) {
        return (
            <div className="text-center py-10 bg-transparent rounded-2xl border border-dashed border-brand-border/20">
    ...
    return (
        <div className="space-y-3">
            ...
            {requests.map(req => (
                <div key={req.id} className="bg-brand-surface p-4 rounded-xl shadow-sm border border-brand-border flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        {req.senderAvatar ? (
                            ...
                            <div className={`w-12 h-12 rounded-full flex items-center justify-center text-white font-bold shadow-sm ${req.type === 'family' ? 'bg-blue-500' :
                                req.type === 'suhba' ? 'bg-purple-500' : 'bg-brand-primary'
                                }`}>
                        ...
                        <div>
                            ...
                            <p className="text-xs text-brand-muted flex items-center gap-1">
                                {req.type === 'family' && <><Home size={12} className="text-blue-500" /> Family Invite</>}
                                {req.type === 'suhba' && <><Users size={12} className="text-purple-500" /> Circle Invite</>}
                                {req.type === 'duo' && <><UserPlus size={12} className="text-brand-primary" /> Partner Request</>}
                            </p>
                            {req.details?.groupName && (
                                <p className="text-xs text-brand-primary font-bold mt-0.5">"{req.details.groupName}"</p>
                            )}
                        </div>
                    </div>
