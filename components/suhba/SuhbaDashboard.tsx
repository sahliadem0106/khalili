...
export const SuhbaDashboard: React.FC = () => {
    ...
    const handleJoinCircle = async () => {
        ...
        try {
            ...
            console.error(error);
            alert(error.message || "Failed to join circle. Check the code and try again.");
        }
    };
    ...
    const handleInviteSubmit = async () => {
        ...
        try {
            ...
            console.error(e);
            alert("Failed to invite. Check ID or connection.");
        }
    };

    if (loading) return <div className="p-10 text-center">Loading Circles...</div>;

    if (circles.length === 0) {
        return (
            <>
                <div className="text-center py-12 bg-white rounded-2xl border border-brand-primary/10 shadow-sm">
                    ...
                    <p className="text-gray-500 max-w-xs mx-auto mb-6 text-sm px-4">
    ...
    return (
        <div className="space-y-6">
                ...
                {circles.map(c => (
                    <button
                        ...
                        className={`px-5 py-2.5 rounded-xl whitespace-nowrap text-sm font-bold transition-all border ${selectedCircle?.id === c.id
                            ...
                            : 'bg-brand-surface/50 text-neutral-500 hover:bg-brand-surface border-white/10 hover:border-brand-primary/20'
                            }`}
                    >
                        {c.name}
                    </button>
            ...
            {selectedCircle && (
                <div className="bg-brand-surface/40 backdrop-blur-md rounded-3xl p-6 shadow-sm border border-white/20">
                        ...
                        <div className="flex gap-2 mb-4">
                            <button
                                ...
                                className={`flex-1 py-2.5 text-xs font-bold rounded-xl border transition-all ${broadcastType === 'custom'
                                    ...
                                    : 'bg-brand-surface/50 text-neutral-500 border-white/10 hover:bg-brand-surface'}`}
                            >
                                Message
                            </button>
                            <button
