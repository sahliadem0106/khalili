...
// --- VISUAL ASSETS ---
import welcomeAnim from '../../src/assets/animations/welcome.json';
import prayerAnim from '../../src/assets/animations/man_praying.json';
import quranAnim from '../../src/assets/animations/quran.json';
import studyAnim from '../../src/assets/animations/study.json';

// Features
import charityAnim from '../../src/assets/animations/charity.json';
import knowledgeAnim from '../../src/assets/animations/knowledge.json';
import tasbihAnim from '../../src/assets/animations/tasbih.json';
import compassAnim from '../../src/assets/animations/compass.json';

// Gender Specific
import braceletBlueAnim from '../../src/assets/animations/blue_bracelet.json';
import braceletPinkAnim from '../../src/assets/animations/pink_bracelet.json';
import womanGesturesAnim from '../../src/assets/animations/woman_gestures.json';
import girlRecitingAnim from '../../src/assets/animations/girl_reciting.json';
import manParkAnim from '../../src/assets/animations/man_park.json';
import manTelescopeAnim from '../../src/assets/animations/man_telescope.json';

// Ghost
import ghostAnim from '../../src/assets/animations/A Ghost.json';
...
export const OnboardingFlow: React.FC<OnboardingFlowProps> = ({ onComplete, initialStep = 0 }) => {
    ...
    const handleGoogleLogin = async () => {
        try {
            ...
            // Check if that happened (means user already has a Firestore profile)
    ...
    const renderStep = () => {
        ...
        switch (step) {
            ...
            case 9: // FORM (Info Request)
                return (
                    <div className="w-full h-full flex flex-col items-center justify-start max-w-md mx-auto px-6 py-10 overflow-y-auto max-h-[85vh]">
                        ...
                        <div className="w-full space-y-6 pb-8">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <input type="number" placeholder={t('onboard_age') as string} value={formData.age} onChange={e => setFormData({ ...formData, age: parseInt(e.target.value) || 0 })} className="bg-white border-2 border-slate-100 text-slate-800 placeholder-slate-400 rounded-2xl p-4 outline-none focus:border-current transition-colors shadow-sm" style={{ borderColor: gender === 'female' ? '#fce7f3' : '#cffafe' }} />
                            ...
                            <div>
                                ...
                                <div className="flex gap-2 mb-2">
                                    <select
                                        ...
                                        className="bg-white border-2 border-slate-100 text-slate-800 rounded-2xl p-4 outline-none focus:border-current shadow-sm"
                                        style={{ borderColor: gender === 'female' ? '#fce7f3' : '#cffafe' }}
                                    >
                                    ...
                                    <input
                                        ...
                                        style={{ borderColor: gender === 'female' ? '#fce7f3' : '#cffafe' }}
                                    />
                                    <button onClick={addSocialLink} className={`w-14 h-14 flex-shrink-0 rounded-2xl font-bold text-white text-2xl flex items-center justify-center transition-colors ${gender === 'female' ? 'bg-pink-400 hover:bg-pink-500' : 'bg-cyan-500 hover:bg-cyan-600'}`}>+</button>
                                </div>
                                <div className="flex flex-wrap gap-2">
                            ...
                            <div className="flex gap-2">
                                <input
                                    ...
                                    style={{ borderColor: gender === 'female' ? '#fce7f3' : '#cffafe' }}
                                />
                                <button onClick={detectLocation} disabled={isDetectingLocation} className="bg-white border-2 border-slate-100 px-5 rounded-2xl text-slate-400 hover:bg-slate-50 transition-colors">
