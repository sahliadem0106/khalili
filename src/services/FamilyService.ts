...
export const FamilyService = {
    ...
    async createFamilyReminderPolicy(
        familyId: string,
        adminId: string,
        message: string,
        scheduleAt: Date
        ...
        const family = await this.getFamily(familyId);
        if (!family) throw new Error("Family not found");
        if (!this.isAdmin(family, adminId)) throw new Error("Only admins can create family reminders");

        return socialNotificationPolicyService.createOneTimePolicy(
            adminId,
            'family',
            { type: 'group', groupId: familyId },
            'Family Reminder',
            message,
            scheduleAt
        );
