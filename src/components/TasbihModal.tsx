...
export const TasbihModal: React.FC<TasbihModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-brand-background w-full h-[95vh] sm:h-[85vh] sm:max-w-md rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col animate-in slide-in-from-bottom duration-300 overflow-hidden border border-white/10">

        {/* HEADER */}
        <div className="bg-brand-surface/90 backdrop-blur-md px-6 py-4 flex justify-between items-center border-b border-white/10 relative z-10">
          <div className="flex items-center space-x-2 rtl:space-x-reverse">
            {view === 'active' && (
              <button onClick={() => setView('setup')} className="text-neutral-400 hover:text-brand-primary transition-colors">
                <Settings2 size={20} />
              </button>
            )}
        ...
        {/* BODY */}
        <div className="flex-1 overflow-y-auto no-scrollbar bg-brand-background">
                  ...
                  <div className="space-y-4">
                    ...
                    {isCustom && (
                      <input
                        ...
                        className="w-full p-4 rounded-2xl bg-brand-surface border border-white/10 focus:ring-2 focus:ring-brand-primary/30 focus:border-brand-primary outline-none shadow-inner text-brand-forest placeholder-neutral-400"
                      />
                    )}
                  </div>
              ...
              {mode === 'combo' && (
                <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-neutral-400 uppercase tracking-widest flex justify-between items-center px-1">
                      <span>{t('tasbih_sequence')}</span>
                      <span className="text-[10px] bg-brand-surface px-2 py-0.5 rounded-full border border-white/10 text-neutral-500">{comboSequence.length} {t('tasbih_steps')}</span>
                    </label>
                      ...
                      {comboSequence.length === 0 ? (
                        ...
                        <div className="space-y-2">
                          {comboSequence.map((step, index) => (
                            <div key={step.id} className="flex items-center justify-between p-3 bg-brand-surface rounded-xl border border-white/5 hover:border-brand-primary/20 group transition-colors shadow-sm">
                              <div className="flex items-center space-x-3 rtl:space-x-reverse">
                                <span className="w-6 h-6 bg-brand-background rounded-full border border-white/10 flex items-center justify-center text-[10px] font-bold text-brand-primary">
                                  {index + 1}
                                </span>
                                <div>
                                  <p className="text-sm font-bold text-brand-forest">{step.label}</p>
                                  <p className="text-[10px] text-neutral-500 font-arabic">{step.arabic}</p>
                                </div>
                              </div>
                              <div className="flex items-center space-x-3 rtl:space-x-reverse">
                                <span className="text-xs font-mono bg-brand-background px-2 py-1 rounded border border-white/10 text-brand-forest font-bold">
                                  x{step.target}
                                </span>
                                <button
                  ...
                  <div className="bg-brand-surface p-4 rounded-2xl border border-white/10 shadow-sm space-y-4">
                    <div className="flex items-center space-x-2 rtl:space-x-reverse text-brand-primary font-bold text-xs uppercase tracking-wide mb-1">
                      <Plus size={14} />
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {PRESETS.slice(0, 5).map(p => (
                        <button
                          ...
                          className={`p-2 rounded-xl border text-[10px] font-bold transition-all ${!isBuilderCustom && builderDhikr.label === p.label
                      ...
                      <button
                        ...
                        className={`p-2 rounded-xl border text-[10px] font-bold transition-all ${isBuilderCustom
                          ? 'bg-brand-primary text-white border-brand-primary'
                    ...
                    {isBuilderCustom && (
                      <input
                        type="text"
                        placeholder={t('tasbih_custom_placeholder')}
                        className="w-full text-sm p-3 rounded-xl border border-white/10 bg-brand-background focus:outline-none focus:ring-1 focus:ring-brand-primary focus:border-brand-primary text-brand-forest"
                    ...
                    <div className="flex items-center space-x-2 rtl:space-x-reverse">
                      {[33, 34, 100].map(t => (
                        <button
                          ...
                          className={`flex-1 py-2 rounded-xl border text-xs font-bold transition-all ${builderTarget === t
                      ...
                      <div className="relative flex-1">
                        <input
                          ...
                          className="w-full py-2 text-center rounded-xl border border-white/10 bg-brand-background text-xs font-bold focus:outline-none focus:border-brand-primary text-brand-forest"
                        />
                    ...
                    <Button fullWidth variant="outline" onClick={addToSequence} className="border-dashed border-brand-primary/30 text-brand-primary hover:bg-brand-primary/5 h-10 text-xs rounded-xl font-bold">
                      {t('tasbih_add_btn')}
                    </Button>
                  </div>
                </div>
          ...
          {view === 'active' && (
            <div className="h-full flex flex-col items-center justify-center p-6 relative">
              <div className="text-center mb-10 space-y-2 min-h-[100px] flex flex-col items-center justify-center">
                <h2 className="text-2xl font-bold text-brand-forest animate-in fade-in slide-in-from-bottom-4 duration-500 font-outfit">
                  {getCurrentDhikr().label}
                </h2>
                <p className="text-xl text-brand-primary font-arabic animate-in fade-in slide-in-from-bottom-3 duration-700 leading-relaxed">
        ...
        {/* FOOTER ACTION */}
        {view === 'setup' && (
          <div className="p-6 bg-brand-surface/90 backdrop-blur-md border-t border-white/10 z-10">
            <Button
              fullWidth
              size="lg"
              onClick={startSession}
              disabled={mode === 'combo' && comboSequence.length === 0}
              className={`shadow-lg shadow-brand-primary/20 bg-brand-primary hover:bg-brand-primary/90 text-white border-none py-4 text-lg font-bold rounded-2xl ${mode === 'combo' && comboSequence.length === 0 ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
