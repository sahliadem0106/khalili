...
export const PrayerDetailModal: React.FC<PrayerDetailModalProps> = ({ prayer, isOpen, onClose, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-brand-forest/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-brand-surface w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl shadow-2xl border border-white/20 max-h-[90vh] overflow-y-auto animate-in slide-in-from-bottom duration-300">

        {/* Header - Forest Green Gradient */}
        <div className="bg-gradient-to-br from-brand-primary to-brand-primary-dark px-5 py-3 border-b border-white/5 sticky top-0 z-10 flex justify-between items-center rounded-t-3xl shadow-md">
          <div className="text-white">
            <h3 className="text-xl font-bold tracking-tight">{['ar', 'ur'].includes(language) ? prayer.arabicName : prayer.name}</h3>
            <div className="flex items-center gap-2 opacity-90">
              <Clock size={12} />
              <p className="text-xs font-medium font-mono">{prayer.time}</p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-8 bg-brand-surface">
          {/* Status Selection */}
          <div className="space-y-3">
            <label className="text-xs font-bold text-brand-muted uppercase tracking-widest px-1">{t('insights')} (Status)</label>
            <div className="grid grid-cols-3 gap-3">
              {[PrayerStatus.Upcoming, PrayerStatus.Jamaah, PrayerStatus.Home, PrayerStatus.Late, PrayerStatus.Missed, PrayerStatus.QadaDone].map((s) => (
                <button
                  ...
                  onClick={() => setStatus(s)}
                  className={`
                    flex flex-col items-center justify-center py-2.5 rounded-xl transition-all duration-200 border
                    ${status === s
                      ? 'bg-transparent border-2 border-brand-primary text-brand-primary font-extrabold shadow-sm scale-[1.02]'
                      : 'bg-brand-subtle border-brand-border text-brand-forest hover:border-brand-primary/20 hover:bg-brand-subtle/80'}
                  `}
                >
          ...
          {isCompleted && (
            <div className="space-y-4 animate-in fade-in slide-in-from-top-2">
              <div className="flex justify-between items-baseline px-1">
                <label className="text-xs font-bold text-brand-muted uppercase tracking-widest">{t('detail_khushu')}</label>
                <span className="text-xs text-brand-primary font-medium">{t('detail_khushu_desc')}</span>
              </div>
              <div className="flex justify-between bg-brand-subtle p-4 rounded-2xl border border-brand-primary/10 shadow-sm">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button key={star} onClick={() => setKhushu(star)} className="transition-transform hover:scale-110 active:scale-95 group">
                    <Star
                      size={32}
                      className={`transition-colors ${star <= khushu ? "fill-brand-secondary text-brand-secondary drop-shadow-sm" : "text-brand-border group-hover:text-brand-secondary/50"}`}
                    />
                  </button>
                ))}
              </div>
          ...
          {isMissedOrLate && (
            <div className="space-y-4 animate-in fade-in slide-in-from-top-2">
              <div className="flex justify-between items-baseline px-1">
                <label className="text-xs font-bold text-red-500 uppercase tracking-widest">{t('detail_barrier')}</label>
                <span className="text-xs text-red-400/80">{t('detail_barrier_desc')}</span>
              </div>
          ...
          {/* Reflection Journal */}
          <div className="space-y-3">
            <label className="text-xs font-bold text-brand-muted uppercase tracking-widest px-1">{t('detail_reflection')}</label>
            <textarea
              value={journal}
              onChange={(e) => setJournal(e.target.value)}
              placeholder={t('detail_reflection_placeholder')}
              className="w-full h-28 bg-brand-subtle rounded-2xl p-4 text-sm border border-brand-border text-brand-forest placeholder:text-brand-muted/70 focus:ring-2 focus:ring-brand-primary/20 focus:border-brand-primary focus:outline-none resize-none shadow-inner"
            />
          </div>

          <div className="pt-2">
            <Button fullWidth size="lg" onClick={handleSave} className="bg-brand-primary text-white hover:bg-brand-primary-dark shadow-primary hover:shadow-primary-lg rounded-xl py-4 font-bold text-base tracking-wide uppercase">
              {t('detail_update_btn')}
            </Button>
          </div>
